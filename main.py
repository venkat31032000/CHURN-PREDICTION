...
"""
vAR_st.write(css1, unsafe_allow_html=True)

col1, col2, col3 = vAR_st.columns([2, 5, 2])
with col1:
    vAR_st.write('')
with col2:
    vAR_st.image("https://raw.githubusercontent.com/venkat31032000/datascience/main/DALL%C2%B7E%202024-04-17%2017.20.13%20-%20A%20sleek%20and%20modern%20logo%20design%20for%20'DataScience'%2C%20featuring%20a%20stylized%20representation%20of%20data%20flow%20and%20analysis.%20The%20logo%20should%20include%20digital%20motif.webp")
with col3:
    vAR_st.write('')

#setting font size and colour for the title 
#by this text-align: centre, we can align the title to the centre of the page
vAR_st.write("""<h1 style='text-align: center; color: white; font-size:29px;'>DataScience Projects</h1>""", unsafe_allow_html=True)

vAR_st.markdown("""<p style='text-align: center; color: blue; font-size:29px;'>By Venkat Ramana Posanipalle</p>""", unsafe_allow_html=True)

vAR_st.markdown("""<p style='text-align: center; color: White; font-size:20px;'><span style='font-weight: bold'>Problem Statement: </span>Develop a Retail Machine Learning Applications (MLOPS): Customer Churn: Who is Going to Churn, When the Churn will Occur, Why it Occurs, and How to Prevent?</p>""", unsafe_allow_html=True)
...
def testing(method):
  ...
  vAR_st.write(churn_probability)
  vAR_st.write('')
  ...
  if vAR_type != 'Select the Problem type':
    if vAR_model != 'Select the Model':
      if vAR_training_data is not None:
        if vAR_training_data.type == 'application/vnd.ms-excel' or 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
          if preview_training:
            vAR_st.dataframe(df_training.head(7))
          vAR_st.write('')  
          if full_table_1:
            vAR_st.dataframe(df_training)          
    ...
    if vAR_model != 'Select the Model':
      if vAR_training_data:
        if vAR_testing_data is not None:
          if vAR_testing_data.type == 'application/vnd.ms-excel' or 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
            if preview_testing:
              vAR_st.write(df_testing.head(7))
            vAR_st.write('')  
            if full_table_2:
              vAR_st.write(df_testing)
            vAR_st.write('')
